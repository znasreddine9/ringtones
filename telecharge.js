...
function notify(event) {
    ...
    var lkey = this.getAttribute("data-lkey");
    if (lkey == 0) {
        var sine = $.ajax({
            url: 'https://statusqueen.com/like',
            type: 'POST',
            data: {
                id: id,
                key: key
            ...
            success: function(data) {
                ...
                $(e).find(".likin").html(likes);
                $(e).find("i").toggleClass("favorite_border favorite");
                $(e).attr('data-lkey', '1');
            }
        });
    }
...
$(".like").on("click", notify);
$(document).ready(function() {
    $(".cbutton").click(function() {
        $(".texux").html("COPY");
        var e = $(this),
            t = $(this).closest(".cruze").find(".getx").text(),
            n = new ClipboardJS('.cbutton', {
                text: function() {
                    return t
                }
            });
        n.on('success', function(t) {
            t.clearSelection(), $(e).closest(".cruze").find(".texux").html("COPIED"), n.destroy()
        }), n.on('error', function(t) {
            t.clearSelection(), $(e).closest(".cruze").find(".texux").html("NO SUPPORT"), n.destroy()
        })
    });
});
